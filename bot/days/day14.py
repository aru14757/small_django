...
class Day14FeedbackStates(StatesGroup):
    awaiting_suggestion = State()
    awaiting_suggestion_4 = State()

feedback_router = Router(name="feedback_router")
scheduler = AsyncIOScheduler()

async def send_scheduled_message(bot: Bot, chat_id, text_message, keyboard=None):
    logging.info(f"Sending message to {chat_id}: {text_message}")
    await bot.send_message(chat_id, text_message, reply_markup=keyboard)

async def dispatch_message(bot: Bot, chat_id, username, message_type):
...
def schedule_day14_messages(bot: Bot):
    users = read_employee_data()
    for user in users:
        ...
        day14_date = first_day + timedelta(days=13)
        ...
        for message_type, send_time in times.items():
            scheduled_time = datetime.combine(day14_date, send_time)
            if scheduled_time > datetime.now():
                scheduler.add_job(dispatch_message, DateTrigger(run_date=scheduled_time),
                                  args=[bot, chat_id, username, message_type])

@feedback_router.callback_query(F.data.in_({"day14_button_1", "day14_button_2", "day14_button_3", "day14_button_4"}))
async def handle_day14_buttons(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        logging.error(f"Error fetching language from server for user {username}: {e}")
        lang = 'ru'  # Use default language if server call fails
    ...
    if message_type in {"day14_button_1", "day14_button_2"}:
        ...
        await state.set_state(Day14FeedbackStates.awaiting_suggestion)
        ...
        await callback.message.edit_text(response_message, reply_markup=keyboard)
        await state.set_state(Day14FeedbackStates.awaiting_suggestion_4)
...
@feedback_router.callback_query(F.data == "day14_suggestion_reply")
...
@feedback_router.callback_query(F.data == "day14_suggestion_reply_4")
async def handle_suggestion_reply_4(callback: CallbackQuery, state: FSMContext):
    lang = await get_language_from_server(callback.from_user.id)
    await callback.message.answer(translate(lang, 'please_write_suggestion'))
    await state.set_state(Day14FeedbackStates.awaiting_suggestion_4)

@feedback_router.message(F.text, StateFilter(Day14FeedbackStates.awaiting_suggestion))
...
@feedback_router.message(F.text, StateFilter(Day14FeedbackStates.awaiting_suggestion_4))
